...
Each class (cat, dog) should have its own folder containing the relevant images. You can use the Kaggle Cats and Dogs Dataset, which can be [downloaded here](https://www.microsoft.com/en-us/download/details.aspx?id=54765).
...
### Output Parameters:
  ...
  - `--model_path PATH`: Where to save the PyTorch model (default: 'cat\_dog\_classifier.pth')
  - `--onnx_path PATH`: Where to save the ONNX model (default: 'cat\_dog\_classifier.onnx')
...
```bash
python main.py --data_dir ./my_images --image_size 224 --batch_size 64 --num_epochs 20 --learning_rate 0.0005 --augmentation
```
